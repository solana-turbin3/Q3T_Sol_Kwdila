...
using SecretHitler;
using SecretHitler.Program;
using SecretHitler.Errors;
using SecretHitler.Accounts;
...
namespace SecretHitler
{
    ...
    namespace Errors
    {
        public enum SecretHitlerErrorKind : uint
        {
    ...
    public partial class SecretHitlerClient : TransactionalBaseClient<SecretHitlerErrorKind>
    {
        public SecretHitlerClient(IRpcClient rpcClient, IStreamingRpcClient streamingRpcClient, PublicKey programId) : base(rpcClient, streamingRpcClient, programId)
        {
        }
        ...
        {
            Solana.Unity.Rpc.Models.TransactionInstruction instr = Program.SecretHitlerProgram.InitPlayer(accounts, levelSeed, programId);
        ...
        {
            Solana.Unity.Rpc.Models.TransactionInstruction instr = Program.SecretHitlerProgram.ChopTree(accounts, levelSeed, counter, programId);
            return await SignAndSendTransaction(instr, feePayer, signingCallback);
        }
        ...
        {
            Solana.Unity.Rpc.Models.TransactionInstruction instr = Program.SecretHitlerProgram.SuperChopTree(accounts, levelSeed, counter, programId);
            return await SignAndSendTransaction(instr, feePayer, signingCallback);
        }

        protected override Dictionary<uint, ProgramError<SecretHitlerErrorKind>> BuildErrorsDictionary()
        {
            return new Dictionary<uint, ProgramError<SecretHitlerErrorKind>>{{6000U, new ProgramError<SecretHitlerErrorKind>(SecretHitlerErrorKind.NotEnoughEnergy, "Not enough energy")}, {6001U, new ProgramError<SecretHitlerErrorKind>(SecretHitlerErrorKind.WrongAuthority, "Wrong Authority")}, };
        }
    }
    ...
    namespace Program
    {
        ...
        public static class SecretHitlerProgram
        {
